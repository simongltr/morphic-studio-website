...
- `screenshot-header-hover.ts` — hovers each header pill target (`.site-header__left`, then each `.site-header__nav a`) and screenshots the top 90px strip at 1440×900 so the sliding hover highlight is visible in stills. Use when an iteration has a header with a JS-driven hover indicator. Usage: `bun run screenshot-header-hover.ts iterations/vN/index.html`.
